<template>
    <div class="main">
        ...
        <div class="login-register-box">
            ...
            <div class="mt10 btn block-btn btn-black" v-if="isAvailabBtn" @click="registSubmit()">{{"注册"}}</div>
            <div class="mt10 btn block-btn btn-gray" v-else>{{"注册"}}</div>
...
import {customerRegister,customerCode} from '~/api/users'
...
export default {
    ...
    data (){
        return {
            ...
            loginTopData:{title:'注册',link:'/users/login',text:'登陆'},
            registUserInfo:{
    ...
    methods:{
        ...
        validateForm (item,key){
            if(item.name === 'phone'){
                if(!GLOBAL.validation.isPhone(item.value) || GLOBAL.validation.isEmpty(this.registUserInfo.code.value) || !GLOBAL.validation.isRegistPassword(this.registUserInfo.password.value)){
                    this.isAvailabBtn = false;
                    return false;
                }
            }else if(item.name === 'code'){
                if(GLOBAL.validation.isEmpty(item.value) || !GLOBAL.validation.isPhone(this.registUserInfo.phone.value) || !GLOBAL.validation.isRegistPassword(this.registUserInfo.password.value)){
                    this.isAvailabBtn = false;
                    return false
                }
            }else if(item.name === 'password'){
                if(!GLOBAL.validation.isRegistPassword(item.value) || !GLOBAL.validation.isPhone(this.registUserInfo.phone.value) || GLOBAL.validation.isEmpty(this.registUserInfo.code.value)){
                    this.isAvailabBtn = false;
                    return false
                }
            }

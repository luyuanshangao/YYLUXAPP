...
{
    /**
     * $status  为1时为EDM  FTP上传
     * ftp链接远程服务器
     * @return resource
     */
    private static function initFTP($status=''){
        if($status == 1){
            // $server = config('ftp_config.DX_FTP_EDN_SERVER_ADDRESS');
            // $port = config('ftp_config.DX_FTP_EDN_SERVER_PORT');
            $server_sbn = config('ftp_edm_config.DX_FTP_EDN_SERVER_ADDRESS');
            $port_sbn = config('ftp_edm_config.DX_FTP_EDN_SERVER_PORT');
            $user_name = config('ftp_edm_config.DX_FTP_EDN_USER_NAME');
            $password = config('ftp_edm_config.DX_FTP_EDN_USER_PSD');
        }else{
    ...
    private static function uploadFile($connect, $romote_file, $local_file){
        $rtn = false;
        Log::record('file'.$romote_file.$local_file);
     ...
     * @return bool
     */
    public static function data_put(array $config,$status=''){

        $connect =  self::initFTP($status);

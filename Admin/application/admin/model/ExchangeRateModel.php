...
class ExchangeRateModel extends Model{
    ...
    public function getDataPaginate($where = array(), $page_size=10,$params = array()){
        return Db::table($this->table_rate)->where($where)->paginate($page_size,false,['query'=>$params]);
    }
    ...
    public function getExchangeRate($id){
        return Db::table($this->table_rate)->where(['id' => $id])->find();
    }
    ...
    public function updateExchangeRate($id,$update){
        return Db::table($this->table_rate)->where(['id' => $id])->update($update);
    }
    ...
    private function _addExchangeRateLog($before_v,$after_v,$rate_id,$field_name,$operation = null){
        ...
        return Db::table($this->table_rate_log)->insert($insert);
    }
     ...
     * @return $this
     */
    public function getLogDataPaginate($where = array(), $page_size=10,$params = array()){
        return Db::table($this->table_rate_log)->where($where)->order('add_time desc')->paginate($page_size,false,['query'=>$params]);
    }
}
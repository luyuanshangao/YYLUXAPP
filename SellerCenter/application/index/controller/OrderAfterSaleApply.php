...
use think\Config;
use think\Log;
class OrderAfterSaleApply  extends Common
{
    /**
     * 售后数据详情
     * @return mixed
    ...
    public function afterSaleApplyDetails(){
        ...
        $this->assign([
            ...
            'ajax_url'=>json_encode([
                ...
                'async_addReturnProductExpressage'=>url('OrderAfterSaleApply/async_addReturnProductExpressage'),
            ]),
        ]);
        return $this->fetch();
    }
...
//        print_r($order_info);
     ...
     // dump($order_info);exit;
        $this->assign([
            'order_info'=>$order_info,
            'after_config_json'=>json_encode($after_config),
            'is_only_refund'=>$is_only_refund,
            'after_config'=>$after_config,
            'ajax_url'=>json_encode([
                'async_submitApply'=>url('OrderAfterSaleApply/async_submitApply'),
            ]),
        ]);
    ...
    public function async_submitApply(){
        ...
        if(empty($after_sale_id)){
            ...
            if($res['code'] == API_RETURN_SUCCESS){
                $after_sale_id =  $res['data'];
            }
        }
        if (isset($after_sale_id) && !empty($after_sale_id)){
            $param['after_sale_id'] = $after_sale_id;
            $param['status'] = 5;
            ...
            if (
                ...
                $chage_desc = "Seller refund operation cancels order";

...
class CommonLib
{
     ...
     */
    public static function createOrderNumner($machine_id = ''){
        $_time = substr(date("Ymd"),2,6);
        $_machine_id = !empty($machine_id)?$machine_id:config("machine_id");
    ...
    public static function rmProductCache($product_id){
        //当redis数量大的时候，不能用keys命令，会导致redis卡顿
        /*
        $redis = new RedisClusterBase();
        //产品详情缓存
...
//            }
     ...
     * @return string
     */
    public static function generatePayToken($order_master_number, $step=6080){
        $abc_arr = ['H','I','A','B','W','X','M','N','O','D','E','Y','C','L','J','K','R','V','F','G','Z','P','Q','S','T','U'];
//        $order_master_number = '180510018204966429';
        $token_flag_1 = mb_substr($order_master_number, 0, 10);
        $str = mb_substr($order_master_number, -8, 8);
//echo $str.'<br>';
//        str_pad(array_sum($order_number_8_arr), 8, "0", STR_PAD_LEFT)
        $flag = $str+$step;
        $flag = str_pad($flag, 8, "0", STR_PAD_LEFT);
        $str_split = str_split($flag);
//print_r($str_split);echo '<br>';
        $str_split[0] = $abc_arr[$str_split[0]];
        $str_split[2] = $abc_arr[$str_split[2]];
        $str_split[4] = $abc_arr[$str_split[4]];
        $str_split[5] = $abc_arr[$str_split[5]];
        $str_split[7] = $abc_arr[$str_split[7]];
        $str_split[] = $abc_arr[rand(0, (count($abc_arr) - 1))];
        $str_split[] = $abc_arr[rand(0, (count($abc_arr) - 1))];
//print_r($str_split);echo '<br>';
        $token_flag_2 = implode('', $str_split);
//echo implode('', $str_split);echo '<br>';
//echo count($abc_arr) - 1;echo '<br>';
//        echo $order_master_number;echo '<br>';
//        echo $abc_arr[rand(0, (count($abc_arr) - 1))].$token_flag_1.$token_flag_2;echo '<br>';
        $pay_token = $abc_arr[rand(0, (count($abc_arr) - 1))].$token_flag_1.$token_flag_2;
        return $pay_token;
    }

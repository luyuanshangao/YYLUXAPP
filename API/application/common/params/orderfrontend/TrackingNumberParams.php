...
class TrackingNumberParams
{
    ...
    public function postItemRules(){
        return [
            ['sku_id','require'], //SKU code包含：整型、字符串 tinghu.liu 20191021
    ...
    /**
     * 接收追踪号数据，同步到订单数据库数据校验(换单用)
     * @return array
     */
    public function syncPostForChangeOfOrderRules(){
        return [
            ['sign','require'],
            ['tracking_number','require'],
            ['old_tracking_number','require'],
            ['order_number','require'],
            ['shipping_channel_name','require'], //具体运输渠道名称
        ];
    }
    ...
    /**
     * 接收追踪号数据，同步到订单数据库数据校验（上传多个追踪号用）
     * @return array
     */
    public function syncPostForMultiRules(){
        return [
            ['sign','require'],
            ['order_number','require'],
            ['data','require'],
    ...
    /**
     * 接收追踪号数据，同步到订单数据库数据校验【产品】（上传多个追踪号用）
     * @return array
     */
    public function syncPostItemForMultiRules(){
        return [
            ['tracking_number','require'],
            ['shipping_channel_name','require'],
            ['item_info','require'],
        ];
    }

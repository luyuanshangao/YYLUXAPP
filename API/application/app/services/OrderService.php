...
//        $this->NocService = new NocService();
    ...
    public function submitOrder($params){
        ...
        if(isset($_cart_info[$_customer_id]) && count($_cart_info[$_customer_id]) > 0){
            ...
            $_params['country'] = $params['country'];
            ...
            if(!isset($_cart_info_res['code']) || $_cart_info_res['code'] != 1){
                ...
                Monlog::write(LOGS_MALL_CART,'error',__METHOD__,'submitOrder'.$_order_master_number,$params,MALL_API.'/mall/product/checkCart','cart no product');
                return $returnData;
            }
            ...
            if(isset($res['code']) && $res['code'] == 200 && isset($res['data'])){
                ...
                $_order_id_arr = isset($res['data'])?$res['data']:array();//获取插入数据库的订单表ID
                ...
                $returnData['msg'] = 'API create order is error!';//'API create order is error!';
                Monlog::write(LOGS_MALL_CART,'error',__METHOD__,'submitOrder'.$_order_master_number,$params,MALL_API.'/mall/product/checkCart','API create order is error!');
                return $returnData;
            }
        }else{
            //购物车为空，返回错误
    ...
    public function checkCart($params){
        ...
        if($_get_address_params['AddressID']){
            //使用用户地址ID的处理
            $Url = MALL_API."/cic/address/getAddress";
            ...
            $_get_address_res['Mobile'] = isset($params['ShippingAddress']['Mobile'])?$params['ShippingAddress']['Mobile']:'';
            ...
            $_get_address_res['Country'] = isset($params['ShippingAddress']['Country'])?$params['ShippingAddress']['Country']:'';
            $_get_address_res['CountryCode'] = isset($params['ShippingAddress']['CountryCode'])?$params['ShippingAddress']['CountryCode']:'';
        }
        //对收货地址进行判断
        $address_res = $this->CommonService->verifyOrderPayAddressParams($_get_address_res);
        ...
        $_data['master']['create_on'] = time();//创建时间
        ...
        foreach ($_cart_info['StoreData'] as $k=>$v){
            ...
            if(isset($v['ProductInfo'])){
                ...
                if(isset($v['isUsedCoupon']['DiscountInfo']['Type']) && $v['isUsedCoupon']['DiscountInfo']['Type']){
                    ...
                    $Url = MALL_API."/cic/MyCoupon/usedCouponByCode";
                    ...
                    if($v['isUsedCoupon']['DiscountInfo']['Type'] == 2){ //这个是什么Coupon
                        ...
                        if($_currency != DEFAULT_CURRENCY){
                            ...
                            $OrderDiscountPriceTmp = sprintf("%.2f", $this->CommonService->calculateRate(DEFAULT_CURRENCY,$_currency,$OrderDiscountPriceTmp,$rate_source));
                        }
                        ...
                        $CouponTmp ['create_on'] = time();
                        //如果当前贷币不是美元，则需要用当前币种转换成美元
                        $CouponTmp ['USD_discount'] = isset($v['isUsedCoupon']['DiscountInfo']['DiscountPrice'])?$v['isUsedCoupon']['DiscountInfo']['DiscountPrice']:0;//以美元为单位的优惠额度
                        $_data['slave'][$k]['coupon'][] = $CouponTmp;
                        $_data['slave'][$k]['order']['coupon_id'] = $v['isUsedCoupon']['CouponId'];//如果使用了coupon则有相应的值
                    }
                }
                    ...
                    if(isset($v2) && count($v2) > 0){
                        foreach ($v2 as $k3=>$v3){
                            ...
                            if((isset($v3['IsBuy']) && $v3['IsBuy'] == 1) && isset($v3['IsChecked'])
                                ...
                                ###SKU_conpon END###################################################
                                $flag = 1;
                                $ProductInfo = $this->CommonService->ProductInfoByID($ProductID,$SkuID,$_lang,$_currency,false);
...
// 								$ShippingInfo = $this->CommonService->countProductShipping($params,$this->productService);
                ...
                if(isset($_data['slave'][$k]['order_item'])){
                    ...
                    $_data['slave'][$k]['order']['create_on'] = time();//创建时间
...
//            $returnData['msg'] = lang('payment_try_again');//'the data is error!';
    ...
    public function checkSC($sc_price,$sc_password,$customer_id,$currency){
        ...
        $Url = MALL_API."/cic/Customer/confirmScPaymentPassword";

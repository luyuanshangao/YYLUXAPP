...
class ProductExtensionService extends BaseService
{
    ...
    public function getRelatedProducts($params){
        ...
        if(config('cache_switch_on')) {
            $products = $this->redis->get(RELATED_PRODUCT_ . $params['product_id'].'_'.$params['lang']);
        }
        if(empty($products)){
            //推荐表取30个SPU
...
//                'endPrice'=>$endPrice,
            ...
            if(!empty($products)){
                $this->redis->set(RELATED_PRODUCT_ . $params['product_id'].'_'.$params['lang'],$products,CACHE_DAY);
            }
        }

...
class Customer extends Model{
    ...
    public function checkLogin($type, $AccountName,$SiteID = 1,$Password){
        ...
        /*由于记录登陆信息保存mongo后之前修改登陆时间方法弃用，在这里登录成功后直接更改最后登录时间，20190411 kevin*/
        if($CustomerS){
            $c_data['LastLoginDate'] = time();
            $db->name('customer')->where($where)->update($c_data);
        }
        return $CustomerS;
    }
    ...
    * */
    ...
    public function GetEmailsByCIDs($ids,$IsSubscriber=''){
        ...
        if($IsSubscriber == true){
            ...
            $res = $db->name('customer')
                    ...
                    ->where($where)
                    ->field("c.EmailUserName,c.EmailDomainName,c.ID,c.Status")
                    ->group("c.ID")
                    ->select();
        }else{
            $where['ID'] = ['in',$ids];
            $res = $db->name('customer')->where($where)->field("EmailUserName,EmailDomainName,ID,Status")->select();
        }
        return $res;
    }
    ...
    public function getEmailsByCID($id,$IsSubscriber=''){
        ...
        if($IsSubscriber == true){
            ...
            $where['ID'] = $id;
            $res = $db->name('customer')->where($where)->field("EmailUserName,EmailDomainName,ID,Status")->find();
        }
        return $res;
    }

    ...
    */
    ...
    public function AffiliateUserStatistics($data){
        ...
        if(!empty($data['PayPalEU'])){
            ...
            $aes = new aes();
            $PayPalEU = $aes->encrypt($data['PayPalEU'],'AffiliateLevel','PayPalEU');//加密邮件前缀
            $where['PayPalEU'] = $PayPalEU;
        }
        $countPage = $this->db->table($this->affiliate_level)->where($where)->count();
        $list['list'] = $this->db->table($this->affiliate_level)
                    ->where($where)
                    ->page($page,$page_size)
                    ->order('RegistrationTimestamp desc')
                    ->field('RCode,CustomerID,PayPalEU,PayPalED,RegistrationTimestamp')
                    ->select();
                    // file_put_contents ('../log/lms/LogisticsUpdateSeller.log',$list['list'].'----------------------', FILE_APPEND|LOCK_EX);

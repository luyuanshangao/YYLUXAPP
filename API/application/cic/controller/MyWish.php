    ...
    public function addWish(){
        try{
            ...
            if(empty($data['CustomerID'])){
                return apiReturn(['code'=>1001,"msg"=>"CustomerID can not be empty"]);
            }
            ...
            if($data['SPU'] >0){
                ...
                if($wish_count>0){
                    //非错误，返回异常错误编号，导致前端商城，记录错误日志，改错误编码，商城特殊判断，add zhongning20190328
                    return apiReturn(['code'=>100000002,'msg'=>'Has been added to the wish list']);
                }
            }else{
...
* */
    ...
    * */
    public function getWishNum(){
        $params = !empty($data)?$data:request()->post();
        $time = time();
        $where=[];
        if (
        ...
        $res = model("MyWish")->getWishNum($where);
        $num=!empty($res['customer_count'])?$res['customer_count']:0;
        return $num;
    }
    ...
    * */
    public function getWishNumber(){
        $params = request()->post();
        if(!empty($params)){
            $res = model("MyWish")->getWishNum($params);
            $num=!empty($res['customer_count'])?$res['customer_count']:0;
            return $num;
        }else{
            Log::record('getWishNumber'.json_encode($params),'error');
        }
        ...
        try{
            ...
            if($res){
                ...
                if(is_array($spus)){
                    ...
                    $ProductLists = $base_api->getWishProductLists(['ids'=>$spus],$Lang);
                    if($ProductLists['code'] == 200){

        ...
        if(!empty($ChannelName)){//网关名称对应枚举
            switch (strtolower($ChannelName)){
                ...
                case 'jcb':
                    $where['GatewayMID'] = 32;
                    break;
                //增加Astropay支付渠道 tinghu.liu 20191017
                case 'astropay':
                    $where['GatewayMID'] = 12;
                    break;
                default:
        ...
        if(empty($ID)){
            ...
            if(!empty($res)){
                foreach ($res as $key=>$value){
                    //如果支付渠道是EGP或聚宝，而且token为空，则删除此记录 Add by 20190804 kevin 跟刘迪确认
                    if(isset($where['GatewayMID']) && ($where['GatewayMID'] ==2 || $where['GatewayMID'] == 8)){
                        $ThirdId_where['CreditCardID'] = $value['ID'];
                        $CreditCardThirdId = $CreditCard->getCreditCardThirdId($ThirdId_where);
                        if(empty($CreditCardThirdId)){
                            unset($res[$key]);
                        }
                    }else{
...
* */
    public function AddCreditCard(){
        ...
        $third_data['GatewayID'] = input("GatewayID");
        array_filter($third_data);
        if( !in_array($third_data['GatewayID'], [2,8]) ){
            $third_data = '';
        }
        $res = $CreditCard->saveCreditCard($data,$third_data);
        if($res!=false){

...
class BaseFunc {
     ...
     * @param array $data
     * @param integer $flag 标识：0-默认，1-来至seller正常编辑类别
     * @return array
     */
    public static function getCategoryInfoWithID($category_id, $data=[], $flag=0){
        $product_class_model = new product_class_model();
        $cinfo = $product_class_model->getInfoWithId($category_id);

        $pid = $cinfo['pid'];
        $class_type = isset($cinfo['type'])?$cinfo['type']:1;
        //来至seller正常编辑类别，且是PDC类别的情况下，需要转换为ERP类别 tinghu.liu 20190730
        if ($flag == 1 && $class_type == 2){
            $cinfo = $product_class_model->getInfoWithId($cinfo['pdc_ids'][0]);
            $pid = $cinfo['pid'];
            $class_type = isset($cinfo['type'])?$cinfo['type']:1;
        }
        ...
        $data[] = $parent_data;
        if ($pid != 0){//不是顶级则继续递归
            return self::getCategoryInfoWithID($pid, $data, $flag);
        }
        return $data;
    }
      ...
      * @param $category_id 子级分类ID
      * @param array $data
      * @param int $flag 标识：0-默认，1-来至seller编辑历史产品同步获取类别数据
      * @return array
      */
    public static function getMapCategoryID($category_id, $data=[], $flag=0){
        $product_class_model = new product_class_model();
        $info = $product_class_model->getInfoWithId($category_id);
        switch ($flag){
            case 1://来至seller编辑历史产品同步获取类别数据
                if ($info['type'] == 1){
                    $data['id_path'] =$info['id_path'];
                    $data['cate_id'] =$info['id'];
                }else{
            ...
            default:
                if(!empty($info['pdc_ids'])){
                    ...
                    $pid = $info['pid'];
                    if($pid != 0){//不是顶级则继续递归
                        return self::getMapCategoryID($pid, $data, $flag);
                    }
                }
                break;
        }

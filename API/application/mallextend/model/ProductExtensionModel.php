...
//ProductDetail_AlsoBought = 3,
     ...
     * 获取spu下的推荐
     */
    public function selectRecommendedSpu($product_id,$type){
        if(is_array($product_id)){
            $where['_id'] = CommonLib::supportArray($product_id);
        }else{
            $where['_id'] = (int)$product_id;
        }
        return $this->db->name(self::productExtension)->where($where)->field('ProductRecommend.'.$type)->select();
    }

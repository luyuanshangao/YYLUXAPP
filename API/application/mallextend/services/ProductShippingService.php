...
use app\mallextend\model\ProductMappingsModel;
...
class ProductShippingService extends BaseService
{
    ...
    protected $Tracking_Information_price = 20;
    ...
    public function countProductShipping($params){
        ...
        $add_shipping = 0.03;
        //人民币切换美元
        $rmbRate = $this->getCurrencyRate(DEFAULT_CURRENCY,'CNY');
...
//                    张恒：2019-1-24 补充 店铺id 777,888 不做这些条件判断
                if($seller_id != 777 && $seller_id != 888) {
...
//        $default = CommonLib::multiArraySort($default,'Cost','SORT_ASC');
     ...
     * @return array
     */
    public function getBatchShippingCost($params){
        $shippingArray = array();
        if(empty($params['spus']) || empty($params['countrys'])){
            return array();
        }
        ...
        foreach($params['spus'] as $spu){
            $request['spu'] = $spu;
            foreach($params['countrys'] as $country){
                $request['country'] = $country;
                $shipping = $this->countProductShipping($request);
                ...
                $shippingArray[$spu][$country] = $shipping;
            }
        }

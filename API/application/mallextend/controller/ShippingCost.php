...
//            $isExist = $shippingModel->getShippingCost(['id' => $product_id]);
            //修改获取逻辑，改为单条 BY tinghu.liu IN 20190128
            $isExist = $shippingModel->getShippingCostV2(['id' => $product_id]);
            if(
                !empty($isExist)
//                && !isset($paramData['is_update']) //暂时放开更新时页删除，为了解决产品运费数据太多导致获取超内存限制问题 BY tinghu.liu IN 20190128
            ){
...
//                        }
     ...
     * 修改产品运费模板
     */
    public function updateAuditProductShipping(){
        $redis = new RedisClusterBase();
        $product_model = new ProductModel();
        $params = input();
        //参数校验

...
//        $data = $model->getCouponByWhere(['CouponId'=>(int)$paramData['CouponId']]);
    ...
    public function addCoupon(){
         ...
         */
        /*指定店铺*/
        if(isset($paramData['DesignatedStore']) && !empty($paramData['DesignatedStore'])){
            foreach ($paramData['DesignatedStore'] as $key=>$value){
                if(empty($value)){
                    unset($paramData['DesignatedStore'][$key]);
                }else{
                    $paramData['DesignatedStore'][$key] = (int)$value;
                }
            }
        }
    ...
    public function deleteCouponCode(){
        $paramData = request()->post();
        //参数校验
        $validate = $this->validate($paramData,(new CouponParams())->deleteCouponCodeRules());
        if(true !== $validate){
            return (['code'=>1002, 'msg'=>$validate]);
        }
        try{
            $model = new CouponModel();
            if ($model->deleteCouponCodeByParams($paramData)){
                return apiReturn(['code'=>200, 'msg'=>'success']);
            }else{
                return apiReturn(['code'=>1003, 'msg'=>'删除失败']);
            }
        }catch (\Exception $e){
            Log::record('deleteCouponCode 程序异常'.$e->getMessage());
            return apiReturn(['code'=>1003, 'msg'=>'程序异常'.$e->getMessage()]);
        }
    }

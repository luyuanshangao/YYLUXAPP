...
class ProductCategory extends Base
{
    ...
    public function getCategoryByID(){
        ...
        if (!isset($params['id'])){
            ...
            $new_data = array();
            //标识：0-默认，1-来至seller编辑类别
            $flag = isset($params['flag'])?$params['flag']:0;
            $data = array_reverse(BaseFunc::getCategoryInfoWithID($params['id'], [], $flag));
            //只要四级
    ...
    public function getMapDataByCategoryID(){
        $params = request()->post();
        if (!empty($params)){
            //标识：0-默认，1-来至seller编辑历史产品同步获取类别数据
            $flag = isset($params['flag'])?$params['flag']:0;
            $data = BaseFunc::getMapCategoryID($params['id'], [], $flag);
            if (empty($data)){
                return apiReturn(['code'=>1006]);
            }else{
                return apiReturn(['code'=>200, 'data'=>$data]);

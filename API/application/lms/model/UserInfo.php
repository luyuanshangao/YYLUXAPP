...
// return $value; return 4;
  ...
  *
  * 追加映射获取数据过滤获取到PDC数据
  * author: Wang  editTime:2018-04-26
  *
  */
  public function product_information($data = array()){
     // $data_array = ['data'=>468987,'status'=>1];
     ...
     foreach ((array)$list as $key => $value) {
           ...
           /********************:2019-01-22 追加映射获取数据*********************************/
           //判断是否需要映射type = 1为erp,type = 2为pdc
           $sum_id = end($category);
           $product_type  =  Db::connect("db_mongodb")->where(['id'=>(int)$sum_id])->field('id,title_en,HSCode,declare_en,title_cn,type,id_path,pdc_ids,SalesUnitType')->name(self::product_class)->find();
           if($product_type['type'] == 2 && !empty($product_type['pdc_ids'])){
                $product_type  =  Db::connect("db_mongodb")->where(['id'=>(int)$product_type["pdc_ids"][0]])->field('id,title_en,HSCode,declare_en,title_cn,type,id_path,pdc_ids,SalesUnitType')->name(self::product_class)->find();
                  $category = explode('-',$product_type["id_path"]);
           }
           /*****************************************************/
           foreach ((array)$category as $ke => $va) {
                $class_id[] = (int)$va;
           }
           $class_name  =  Db::connect("db_mongodb")->where(['id'=>['in',$class_id]])->field('id,title_en,HSCode,declare_en,title_cn,type,id_path')->name(self::product_class)->select();
           $sum = count($class_name);
           $declare_en  = end($category);

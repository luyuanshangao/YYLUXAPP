...
class Article extends Model
{
    ...
    public function getList($where,$page_size=10,$page=1,$path='')
    {
        ...
        $res = $this->db->name("article")
            ...
            ->field("a.article_id,a.image,a.article_title,a.excerpt,a.cate_id,c.cate_name_cn,
                     c.cate_name_en,a.href,a.keywords,a.status,a.add_time,header_image,add_author")
    ...
    public function getInfo($where=''){
        $res = $this->db->name("article")
            ...
            ->where($where)
            ->field("a.article_id,a.image,a.article_title,a.excerpt,a.cate_id,c.cate_name_cn,
                     c.cate_name_en,a.content,a.href,a.keywords,a.status,a.add_time,a.update_time,header_image,add_author")
            ->find();
    ...
    /*获取文章ID*/
    public function getArticleIds($cate_id = ''){
        $where['status'] = 1;
        $where['delete_time'] = ['eq',0];
        if(!empty($cate_id)){
            $where['cate_id'] = ['IN',$cate_id];
        }
        $res = $this->db->name("article")->where($where)->column("article_id");
        return $res;
    }
}

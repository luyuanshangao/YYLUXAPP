...
class VerificationCode extends Model{
     ...
     * 更改验证码
     * */
    public function updateVerificationCode($where,$update_data){
        $res =  $this->db->name('verification_code')->where($where)->update($update_data);
        return $res;
    }
     ...
     * 获取验证码条数
     * */
    public function getVerificationCodeCount($where){
        $res =  $this->db->name('verification_code')->where($where)->count();
        return $res;
    }
}
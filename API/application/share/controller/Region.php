...
class Region extends Base
{
    ...
    public function getRegionList(){
        ...
        if(empty($Code) && empty($Name) && empty($id)){
            /*调用获取数据接口*/
            //不是重点国家
            $res1 = $this->regionModel->getRegion(['ParentID'=>$ParentID,'isFocus'=>2]);
            //重点国家
            $res2 = $this->regionModel->getFocusCountry();
            //打标签，区分最后一个重点国家，方便前端JS区分
            $arr = array_slice($res2,-1,1,true);
            $lastKey = key($arr);
            $res2[$lastKey]['isLastFocus'] = 1;
            //合并
            $newCountry = array_merge($res2,$res1);
            if(is_array($newCountry) && !empty($newCountry)){
                $data = array();
                ...
                foreach ($newCountry as $key => $value){
                    ...
                    //标签
                    if(isset($value['isLastFocus'])){
                        $data[$key]['isLastFocus'] = 1;
                    }
                }
            }
        }else{/*如果有传参*/
        ...
        //return $data;
        return $this->moveElementEnd($data,"Other");
    }
    /**
     * 获取商城头部国家数据
     */
 ...
 * AddTime:2019-03-27
 * */
    function moveElementEnd($arr,$element)
    {

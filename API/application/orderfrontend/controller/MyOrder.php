 ...
 * */
    ...
    * */
     ...
     * @return mixed
     */
    public function getBlockChainOrderList(){
        $paramData = request()->post();
...
//        }
        /*订单状态*/
//        if(isset($paramData['order_status'])){
//            $where['order_create_on'] = $paramData['order_status'];
//        }
        if(!empty($paramData['create_on_start']) && !empty($paramData['create_on_end'])){
            $where['o.create_on'] = ["between",[strtotime($paramData['create_on_start']),strtotime($paramData['create_on_end'])]];
        }else{
            /*成交开始时间*/
            if(!empty($paramData['create_on_start'])){
                $where['o.create_on'] = ['gt',strtotime($paramData['create_on_start'])];
            }
            /*成交结束时间*/
            if(!empty($paramData['create_on_end'])){
                $where['o.create_on'] = ['lt',strtotime($paramData['create_on_end'])];
            }
        }
        $page_size = input("post.page_size",20);
        $page = input("post.page",1);
        $path = input("post.path");
        $order = isset($paramData['order']) ? $paramData['order'] : "o.create_on desc";
        $page_query = isset($paramData['page_query']) ? $paramData['page_query'] : array();
        $list = model("OrderModel")->getBlockChainOrderList($where,$page_size,$page,$path,$order,$page_query);
        return apiReturn(['code'=>200,'data'=>$list]);
    }
}

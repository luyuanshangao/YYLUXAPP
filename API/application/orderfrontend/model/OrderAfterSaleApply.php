...
class OrderAfterSaleApply extends Model{
    ...
    public function saveOrderAfterSaleApply($data,$where=''){
        if(!isset($data['after_sale_id']) && empty($where)){
            $tran = $this->db->transaction(function () use ($data,$where){
                //如果已经提交了售后申请，状态为，1申请待处理、2待买家发货、3待卖家收货、6仲裁处理中、9退款中，则不能再次提交审核 20190412 kevin
                $apply_where['order_number'] = $data['order_number'];
                $apply_where['status'] = ['in','1,2,3,6,9'];
                $apply_data = $this->db->table($this->order_after_sale_apply)->where($apply_where)->find();
                if (!empty($apply_data)){
                ...
                $create_ip = isset($data['create_ip'])?$data['create_ip']:'';
                unset($data['item']);
                ...
                if($item){
                    ...
                    if(!isset($data['initiator']) || $data['initiator'] == 1){
                        $update_order['customer_id'] = !empty($data['customer_id'])?$data['customer_id']:"";
                        $update_order['order_number'] = !empty($data['order_number'])?$data['order_number']:"";
                        $update_order['order_status'] = 920;//售后处理中
                        $update_order['change_reason'] = !empty($data['remarks'])?$data['remarks']:"";
                        $update_order['create_ip'] = $create_ip;
                        $ChangeOrderStatus = controller("orderfrontend/Order")->realChangeOrderStatusNew($update_order);
                        if(empty($ChangeOrderStatus['code']) || $ChangeOrderStatus['code'] != 200){
                            /*当订单状态改变状态时，删除售后订单 20190416 kevin*/
                            $this->db->table($this->order_after_sale_apply)->where(['after_sale_id'=>$res])->delete();
                            Log::write("saveOrderAfterSaleApply realChangeOrderStatus Error,update_order:".json_encode($update_order));
                        }
                        return $res;
                    }else{
                        return $res;
  ...
  * */
     ...
     */
    public function order_after_sale_apply($data){
         $where = [];
         ...
         $result = $this->db->table($this->order_after_sale_apply)->where($where)->find();
         if($result){
             return apiReturn(['code'=>200,'data'=>$result]);
         }else{
             return apiReturn(['code'=>1002,'msg'=>'空']);
         }
     ...
     * @return [type] [description]
     */
    public function order_after_sale_apply_item($data){
         return $this->db->table($this->order_after_sale_apply_item)->insert($data);
    }
    /**
     * 退款订单检测
     * [order_after_sale_apply_item description]
     * @return [type] [description]
     */
    public function OrderDetection($data=array()){
        $data['payment_status'] = ['in','120,200,300,400'];
        $list = $this->db->table($this->order)->field('order_number')->where($data)->select();
        if(!empty($list)){
            return apiReturn(['code'=>200,'data'=>$list]);
        }else{
            return apiReturn(['code'=>1002,'msg'=>'这些订单没有达到退款要求']);
        }
    }
    /**
     * 退款订单部分sku删除个数
     ...
     * @return [type] [description]
     */
    public function SomeSkuRefunds($data = array()){
          $data['payment_status'] = ['in','120,200,300,400'];
          $list = $this->db->table($this->order)->field('order_number,order_id')->where($data)->select();
          if(!empty($list)){
              foreach ($list as $k => $v) {
                 if(!empty($v['order_number']) ){
                     $list_sku = $this->db->table($this->order_item)->field('product_nums,sku_num,shipping_fee,shipping_model')->where(['order_id'=>$v['order_id']])->select();
                     $list[$k]['list_sku'] = $list_sku;
                 }
              }
              return apiReturn(['code'=>200,'data'=>$list]);
          }else{

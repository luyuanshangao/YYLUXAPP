 ...
 */
  ...
  public function export(){
      ...
      // pr($list);exit;
       if(empty($list)){
           $this->admin->table($this->screening_management)->where(['status'=>4,'id'=>$screening_management['id']])->update(['status'=>5]);
           return;
       }else{
     ...
     *
     */
    public function EDM_FTP($name='OrderDdata'){
        $result =  $this->admin->table($this->screening_management)->where(['status'=>5])->order('add_time ASC')->find();//dump($result);
        if(!$result){
            return;
        }
        ...
        $config = [
                ...
                'dirPath'=>$ftp_config['DX_FTP_FILE_URL'].date('Y-m-d',time()).'/', // ftp保存目录
                'romote_file'=>$ftp_config['DX_FTP_FILE_URL'].date('Y-m-d',time()).'/'.$result['TopicName'].'_1.xls',//保存文件的名称
                'local_file'=>$ftp_config['DX_FILE_URL'].$result['TopicName'].'_1.xls', // 要上传的文件
                // 'local_file'=>$ftp_config['DX_FILE_URL'].$result['TopicName'].'_'.time().'.xls', // 要上传的文件
            ];

        $ftp = new FTPUpload();//pr($ftp);exit;
        $upload = $ftp->data_put($config,1);
        if($upload){
            $this->admin->table($this->screening_management)->where(['status'=>5,'id'=>$result['id']])->order('add_time ASC')->update(['status'=>2]);//dump($result);
            // redis()->del(DX_ORDER_QUERY.'_'.$result['id']);//删除key
        }
    }
}
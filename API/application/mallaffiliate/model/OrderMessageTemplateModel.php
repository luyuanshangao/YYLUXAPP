...
class OrderMessageTemplateModel  extends Model{
    ...
    public function getOrderMessageTemplate($where){
        $res = Db::connect("db_admin")->table($this->table)->where($where)->order("id","DESC")->select();
    ...
    public function getOrderMessageTemplateInfo($where){
        $res = Db::connect("db_admin")->table($this->table)->where($where)->find();
    ...
    public function saveOrderMessageTemplate($data){
        if($data['type'] == 2 && $data['status'] == 1){
            ...
            Db::connect("db_admin")->table($this->table)->where($update_where)->update(['status'=>2]);
        }
        if(!empty($data['id'])){
            $where['id'] = $data['id'];
            return Db::connect("db_admin")->table($this->table)->where($where)->update($data);
        }else{
            return Db::connect("db_admin")->table($this->table)->insertGetId($data);
        }
    }
    ...
    public function deleteOrderMessageTemplate($where){
        return Db::connect("db_admin")->table($this->table)->where($where)->delete();
    }
}
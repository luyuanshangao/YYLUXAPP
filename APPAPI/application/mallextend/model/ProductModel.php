    ...
    public function updateProduct($params){
        ...
        if (isset($params['Skus'])) {
            ...
            foreach ($params['Skus'] as $sk => $sku) {
                //原sku
                $params['Skus'][$sk]['_id'] = isset($sku['_id']) ? (int)$sku['_id'] : '';
                $params['Skus'][$sk]['Inventory'] = isset($sku['_id']) ? (int)$sku['Inventory'] : 0;
                $params['Skus'][$sk]['SalesPrice'] = isset($sku['SalesPrice']) ? (double)$sku['SalesPrice'] : 0;
                //新增sku
                $newSku = isset($sku['_id']) ? $sku['_id'] : '';
                if (empty($newSku)){
                    $this->redis->incr('product_sku_id_incr');
                    $product_sku_id = $this->redis->get('product_sku_id_incr');
                    $params['Skus'][$sk]['_id'] = (int)$product_sku_id;
                    $params['Skus'][$sk]['Inventory'] = (int)$sku['Inventory'];
                    $params['Skus'][$sk]['SalesPrice'] = (double)$sku['SalesPrice'];
                }
                //处理批发价格
                if(isset($sku['BulkRateSet']['Discount']) && !empty($sku['BulkRateSet']['Discount'])){
        ...
        if($ret){
            ...
            if (isset($params['Skus'])) {
                if ($product_sku_id != $incrment['SubSKU']) {
                    $updateResult = $this->updateAutoIncrement(
                        ['SubSKU' => (int)$product_sku_id], ['SKU' => (int)$incrment['SKU'], 'SubSKU' => (int)$incrment['SubSKU']]);
//                    if (!$updateResult) {
//                        return apiReturn(['code' => 1000000023, 'msg' => '更新产品自增ID失败']);
//                    }
                }
            }
    ...
    public function updateProductCommission($store_id, $up_data){
        //查询更新产品是否存在
        $proudct = $this->db->name('product')->where(['StoreID' =>(int)$store_id])->select();
        if(empty($proudct)){
            return false;
        }
     ...
     * @return bool
     */
    public function updateProductCommissionByFirstCategory($store_id, $first_category_id, $up_data){
        //查询更新产品是否存在
        $proudct = $this->db->name('product')->where(['StoreID' =>(int)$store_id])->select();
        if(empty($proudct)){
            return false;
        ...
        $up_data['CommissionType'] = (int)$up_data['CommissionType'];
        $up_data['Commission'] = (float)$up_data['Commission'];
        $ret = $this->db->name('product')->where(['StoreID' =>(int)$store_id, 'FirstCategory'=>(int)$first_category_id])->update($up_data);
        if(!$ret){
            return false;
        }
        return true;
...
//        }
    ...
    public function getProductByIDs($params){
        ...
        $query->field(['Title','Skus.Inventory','Skus._id','Skus.Code','Skus.SalesPrice','SalesRank','BrandId','RewrittenUrl','ImageSet.ProductImg',
            'FirstCategory','PackingList.Weight','HSCode','LogisticsLimit','AddTime','EditTime','ProductStatus','DeclarationName',
            'SecondCategory','ThirdCategory','FourthCategory','IsHistory','Skus.SalesAttrs','Skus.UnitCost','BrandName','LowPrice','HightPrice',
            'DiscountLowPrice','DiscountHightPrice','HightDiscount','IsActivity','Gtins','SalesUnitType','Supplier'
        ]);
...
//        $ret = $query->where($where)->field(['_id','Skus._id','Skus.SalesPrice','LowPrice'])->select();
    ...
    public function queryProduct916($params){
        ...
        $where['ProductStatus'] = ['in',[self::PRODUCT_STATUS_SUCCESS,self::PRODUCT_STATUS_SUCCESS_UPDATE]];
//        $where['RewrittenUrl'] = ['like','#'];
        $query->where($where);
        //$query->order('AddTime','desc');
        //搜索字段
        $query->field(['_id','IsHistory','RewrittenUrl','LowPrice','HightPrice','Skus.SalesPrice','Skus.BulkRateSet','Skus._id','CategoryPath','AddTime','EditTime']);
        return $query->paginate($page_size,false,['type' => 'Bootstrap', 'page' => $page])->toArray();
    }

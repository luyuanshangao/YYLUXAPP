...
class Affiliate extends Model{
     ...
     * 获取列表
     * */
    public function getBannerList($where,$page_size,$page,$path,$order){
        $where['IsDelete'] = (int)0;
        $res = $this->db->table($this->table)->where($where)->order($order[0], $order[1])->paginate($page_size,false,['type' => 'Bootstrap', 'page' => $page,'path' => $path,'query'=>$where]);
        $Page = $res->render();
        $data = $res->toArray();
        $data['Page'] = $Page;
        return $data;

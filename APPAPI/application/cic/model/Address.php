...
class Address extends Model{
    ...
    * */
    public function getDefaultAddres($CustomerID){
        ...
        $where['Country'] = ["NEQ",""];
        $res = $db->name("address")->where($where)->field("AddressID,CustomerID,ContactName,EmailUserName,EmailDomainName,FirstName,LastName,Street1,Street2,City,Province,Country,CountryCode,Mobile,Phone,Email,PostalCode,IsDefault,CardID,ProvinceCode,CityCode,CreateTime,UpdateTime,DeleteTime,CPF")->find();
        return $res;
    }
    ...
    * */
    public function getAddress($where){
        $db = Db::connect('db_cic');
        $res = $db->name("address")->where($where)->order("AddressID","DESC")->field("AddressID,CustomerID,ContactName,EmailUserName,EmailDomainName,FirstName,LastName,Street1,Street2,City,Province,Country,CountryCode,Mobile,Phone,Email,PostalCode,IsDefault,CardID,ProvinceCode,CityCode")->find();
        return $res;
    }
    ...
    * */
    public function getCardAddress($CardID){
        $db = Db::connect('db_cic');
        $where['CardID'] = $CardID;
        $res = $db->name("address")->where($where)->field("AddressID,CustomerID,ContactName,EmailUserName,EmailDomainName,FirstName,LastName,Street1,Street2,City,Province,Country,CountryCode,Mobile,Phone,Email,PostalCode,IsDefault,CardID")->find();
        return $res;
    }
}
...
//        $this->NocService = new NocService();
    ...
    public function getCartInfo($Uid,$Currency,$Country,$Lang,$GuestId,$UserName){
        ...
        $IsHasNocNoc = 0;
        /*2、遍历购物车里的信息，*/
        $isDifferentStoreID = 0;
        Monlog::write(LOGS_MALL_CART,'info',__METHOD__,$Uid,$CartInfo,MALL_API.'/checkoutService/getCartInfo1');
        if($CartInfo){
            $this->CommonService->processCartProduct($CartInfo,$Currency,$Country,$Lang,$Uid,'checkout',$GlobalShipTo,$IsHasNocNoc,$UserName,2, $isDifferentStoreID);
        }
        Monlog::write(LOGS_MALL_CART,'info',__METHOD__,$Uid,$CartInfo,MALL_API.'/checkoutService/getCartInfo2');
            ...
            foreach ($CartInfo[$Uid]['StoreData'] as $k=>$v){
                ...
                if(isset($v['isUsedCouponDX'])){
                    if(isset($v['isUsedCouponDX']['DiscountInfo']['DiscountPrice'])){
                        ...
                        //20190711 checkout优惠券已经是当前币种不用再转换
...
//                                $TmpPrice = sprintf("%.2f", $TmpPrice);
     ...
     */
    public function getPayType($Currency,$Country,$version=0){
        $NocNocPayInfo = null;
        ...
        for ($i = 0; $i < count($Data['data']['PayType']); $i++){
            foreach ($Data['data']['PayType'] as $k => $v){
                //如果是支付渠道是Astropay，需要将拼接卡信息 tinghu.liu 20190325
                if ($v['channel']['channelName'] == 'Astropay'){
                    $v['AstropayCardData'] = $this->getAstroPayCardInfo($Currency, $Country, $v['payname']);
                }else{
                    $v['AstropayCardData'] = (object)[];
                }
                ...
                //新版才有ideal支付
                if(($version>11)&&strtolower($v['payname']) == 'ideal'){
                    $TmpPayType[5] = $v;
                    $data=(new PaymentService())->getIdealBank();
                    $TmpPayType[5]['BankList'] = !empty($data['data'])?$data['data']:[];
                }
            }
        }
        ksort($TmpPayType);
    ...
    public function getAstroPayCardInfo($Currency,$Country,$PayType){
        ...
        switch ($Currency){
            case "MXN":
                ...
                //墨西哥币种支付的信用卡支付
                $BankList = ['VI','MC'];
                //增加银行转账和Boleto tinghu.liu 20191030
                if($PayType == 'Transfer-Astropay'){
                    //银行卡转帐支持
                    $BankList = ['SE', 'BV', 'BQ', 'SM'];
                ...
                break;
            case "ARS":
                //名称由‘DNI’修改为‘CUIT/CUIL’ tinghu.liu 20190529
                $PayTypeInputName = 'CUIT/CUIL';
                //阿根廷币种支持的信用卡支付
            ...
            case "BRL":
                ...
                //巴西里尔支持的信用卡支付 ['VI','MC','EL','DC','HI','ML','AE'];
                $BankList = ['VI','MC','EL','HI','ML','AE'];
                if($PayType == 'Transfer-Astropay'){
                    //银行卡转帐支持
                    //去掉H，因为新支付系统不支持 tinghu.liu 20190909 ['SB','CA','I','BB','B', 'H'];
                    $BankList = ['SB','CA','I','BB','B'];
                }else if($PayType == 'Boleto-Astropay'){
                    //线下付款支付的银行
                    $BankList = ['BL'];
                }
                break;
            //增加印度币种 tinghu.liu 20191030
            case "INR":
                $PayTypeInputName = 'PAN';
                //巴西里尔支持的信用卡支付 ['VI','VD','MC', 'MD','AE', 'RU'];，因为VI,VD和MC,MD图标一致，所以去掉其中一个
                $BankList = ['VI', 'MD','AE', 'RU'];
                if($PayType == 'Transfer-Astropay'){
                    //银行卡转帐支持
                    $BankList = ['PW','NB','UI'];
                }
    ...
    /**
     * 【Astropay新接口】Astropay支付方式转换【为了传给payment，因为数据库保存的类型和payment不一致】
     * @param $payment_method
     * @return string
     */
    public function astropayPaymentMethodTransV2($payment_method){
        $result = $payment_method;
        $payment_method = strtolower($payment_method);
        switch ($payment_method){
            case 'creditcard': //CreditCard
            case 'creditcard-astropay': //CreditCard-Astropay
                $result = TRANSACTION_TYPE_ASTROPAY_CREDIT_CARD;
                break;
            case 'boleto-astropay': //Boleto-Astropay
                $result = TRANSACTION_TYPE_ASTROPAY_CASH;
                break;
            case 'transfer-astropay': //Transfer-Astropay
                $result = TRANSACTION_TYPE_ASTROPAY_ONLINE;
                break;
            default:break;
        }
        return $result;
    ...
    public function editUserAddress($Uid,$Params){
        $Url = CIC_APP."/cic/address/saveAddress";
        $Data = doCurl($Url,$Params,null,true);
        return $Data;
    }
    ...
    public function addToCartBuyNow($Params){
        ...
        //$ShipTo 区域定价 added by zhongning in 20190523
        ...
        if(!isset($ProductInfo['code']) || $ProductInfo['code'] != 200 || !isset($ProductInfo['data']) || count($ProductInfo['data']) < 1){
            Log::record('addToCartBuyNow'.$ProductID.'$SkuID'.$SkuID);
            //没有找到数据
            $Return['code'] = 0;
        ...
        if($ProductInfo){
            ...
            if(isset($_product_price_info['code']) && ($_product_price_info['code']==1)){
                ...
                log::record('addToCartBuyNowProductInfo'.json_encode($ProductInfo).'sku'.$SkuID,'error');
                log::record('addToCartBuyNow$_product_price_info'.json_encode($_product_price_info),'error');
            }
        }

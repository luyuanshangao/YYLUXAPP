...
class NocService extends BaseService
{
    ...
    public function __construct(){
        ...
        $this->LoginService = new LoginService();//用户数据相关的类
    }
    ...
    public function claNocNocData($params,&$_cart_info,$from=0){
        $_customer_id = isset($params['customer_id'])?$params['customer_id']:0;
        ...
        if ($from == 1){
            ...
            if (isset($_cart_info[$_customer_id]['nocdata']))
                unset($_cart_info[$_customer_id]['nocdata']);
            return true;
        }
        $_tax_id = isset($params['tax_id'])?$params['tax_id']:0;
        $zipcode = isset($params['zipcode'])?$params['zipcode']:'';
        //币种
        $_currency = isset($params['currency']) && !empty($params['currency'])?$params['currency']:Cookie::get('DXGlobalization_currency');
        $rate_source = [];
        ...
        if(isset($_cart_info[$_customer_id]['StoreData'])){
            ...
            /**
            "address":{
            "recipient_name":"Julian Casiello",
            "phone":"55 93 991011669",
            "street":"Balcarce 740",
            "street_2":"5 D",
            "zipcode":"68180400",
            "region":"Para",
             ...
             *
             **/
//            $_addres_data = json_decode(cookie('currAddressMessage'),true);

            /**
             * 地址信息修改，使用地址ID的形式获取数据库对应数据 start
             * tinghu.liu 20190725
             */
            $customer_address_id = $address_id;

            $_addres_data = $this->CommonService->getUserAddressData($_customer_id, $customer_address_id);

            if (empty($_addres_data)){
                $_address_params_log = ['customer_id'=>$_customer_id, 'customer_address_id'=>$customer_address_id];
                Log::record('$_addres_data is empty:'.$log_key.json_encode($_address_params_log), Log::NOTICE);
                if ($from == 3){
                    return 'The request failed. Please try again.';
                }elseif($from == 4 || $from == 2){
                    return ['code'=>0, 'msg'=>'The request failed. Please try again.'];
                }else {
                    return false;
                }
            }
            /** 地址信息修改，使用地址ID的形式获取数据库对应数据 end **/

            $adress['recipient_name'] = (isset($_addres_data['FirstName'])?$_addres_data['FirstName']:'').' '.(isset($_addres_data['LastName'])?$_addres_data['LastName']:'');
            $adress['phone'] = (isset($_addres_data['Mobile']) && !empty($_addres_data['Mobile']))?$_addres_data['Mobile']:((isset($_addres_data['Phone']) && !empty($_addres_data['Phone']))?$_addres_data['Phone']:'');
            $adress['street'] = isset($_addres_data['Street1'])  && !empty($_addres_data['Street1'])?$_addres_data['Street1']:'';
            $adress['street_2'] = isset($_addres_data['Street2'])  && !empty($_addres_data['Street2'])?$_addres_data['Street2']:'';
            $adress['region'] = isset($_addres_data['Province'])  && !empty($_addres_data['Province'])?$_addres_data['Province']:'';
            $adress['city'] = isset($_addres_data['City'])  && !empty($_addres_data['City'])?$_addres_data['City']:'';
            $adress['country'] = isset($_addres_data['CountryCode'])  && !empty($_addres_data['CountryCode'])?$_addres_data['CountryCode']:'';
            $adress['zipcode'] = isset($_addres_data['PostalCode'])  && !empty($_addres_data['PostalCode'])?$_addres_data['PostalCode']:'';
            $_nocnoc_request_params['address'] = $adress;
            //对地址进行参数校验
            $_verify_address_res = $this->CommonService->verifyAddressForNocQuote($adress);
            if (
                !isset($_verify_address_res['code'])
                || $_verify_address_res['code'] != 200
            ){
                Log::record('verifyAddressForNocQuote:'.$log_key.json_encode($_verify_address_res));
                if (isset($_cart_info[$_customer_id]['nocdata']))
                    unset($_cart_info[$_customer_id]['nocdata']);
                if ($from == 3){
                    return $_verify_address_res['msg'];
                }elseif($from == 4 || $from == 2){
                    return ['code'=>0, 'msg'=>$_verify_address_res['msg']];
                }else {
                    return false;
                }
            }
...
//                'full_name' => 'DX',
                'full_name' => $adress['recipient_name'],
                'country' => $_country,
            );
            /**购物车询价无需传入**/
            if($_tax_id && $from != 1){
            ...
            if(empty($dx_noc_class_map)){
                logService::write(LOGS_MALL,'error',__METHOD__,__FUNCTION__,null,null,'dx map noc class empty!');
                return false;
            }*/
                ...
                foreach ($v['ProductInfo'] as $k1=>$v1){
                    foreach ($v1 as $k2=>$v2){
                        if(
                            ...
                            $noc_class_id = $this->getNocClass($v2['ProductID'],$v2['SkuID'], $_customer_id);
            ...
            if($flag){
                ...
                Log::record('NOCNOC服务接口$_nocnoc_request_params'.json_encode($_nocnoc_request_params));
                $_nocnoc_data = doCurl(API_NOC_URL,$_nocnoc_request_params,null,true,$_post_header,$_user_agent);
                Log::record('NOCNOC服务接口'.json_encode($_nocnoc_data));
                ...
                if(is_array($_nocnoc_data)){
                    ...
                    if (isset($_cart_info[$_customer_id]['nocdata']))
                        unset($_cart_info[$_customer_id]['nocdata']);
                    //判断NOCNOC，如果返回是字符串时表名是错误信息，直接展示给用户 BY tinghu.liu IN 2190129
                    $_msg = is_string($_nocnoc_data)?$_nocnoc_data:'Inquiry failed, please try again.';
                    if ($from == 3){
                        return $_msg;//'System error.';
                    }elseif($from == 4 || $from == 2){
                        return ['code'=>0, 'msg'=>$_msg/*'System error.'*/];
                    }else{
                        return false;
                    }
                }
    ...
    public function editCustomerTaxIdService($uid,$taxid){
        $result = $this->LoginService->editCustomerTaxIdService($uid,$taxid);
        return $result;
    }
}
...
class ProductClassService extends BaseService
{
     ...
     * @return array
     */
    public function getIntegrationClass($params,$list){
        $this->classModel = new ProductClassModel();
        $data = array();
        //判断是否有缓存
        if(config('cache_switch_on')){
            $data = $this->redis->get('APP_INTEGRATION_CLASS'.$params['lang']);
        }
        if(empty($data)){
            $result = $this->classModel->integrationClassName(['language' => $params['lang']]);
            if(empty($result)){
                //其他没有数据，默认取英文
                $result = $this->classModel->integrationClassName(['language' => DEFAULT_LANG]);
            }
            ...
            foreach($result as $key => $productClass) {
                ...
                if(!empty($array[0])){
                    $html['id']=$array[0];
                    $html['pid'] = 0;
                    $html['title_en'] = $productClass['className'];
                    if(!empty($list[$classId])){
                        $html['classimg'] = !empty($list[$classId]['classimg'])?$list[$classId]['classimg']:'';
                    }else{
                        $html['classimg'] = '';
                    }
                    $data[]=$html;
            ...
            $this->redis->set('APP_INTEGRATION_CLASS'.$params['lang'],$data,CACHE_FIVE_MIN);
        }
...
//                        pr($third);die;
     ...
     * @return array
     */
    public function getClassTree($params,$lang){
        $this->classModel = new ProductClassModel();
        $result = array();
        $data = $this->classModel->getClassByPid(['pid'=>(int)$params['class_id'],'lang'=>$lang]);
        if(!empty($data)){
            foreach($data as $key => $secData){
                $data[$key]['children'] = array();
                $thrData = $this->classModel->getClassByPid(['pid'=>(int)$secData['id'],'lang'=>$lang]);
                if(!empty($thrData)){
                    $data[$key]['children'] = $thrData;
                }
            }
        }
     ...
     * @return mixed
     */
    public function getInClass($params){
        $params['lang'] = isset($params['lang']) ? $params['lang'] : DEFAULT_LANG;
        $id = $params['id'];
        $this->classModel = new ProductClassModel();
        $result = array();

...
class CouponModel extends Model{
    ...
    public function getAvailableCoupon($params){
        ...
        $couponArray = $query->where($where)
            ...
            Description,CouponTime,CouponStatus,DesignatedStore")
            ->select();
        if($params['IsAutoCouponGetCouponCode']){
            if(!empty($couponArray)){
                ...
                foreach($couponArray as $k=>$v){
                    ...
                    //20181221 过滤活动策略：1-线上活动、2-线下活动。【为了兼容没有活动策略的老数据，需要在此循环过滤】
                    /*if(isset($params['ActivityStrategy']) && $params['ActivityStrategy']){
                        if (
                            isset($v['ActivityStrategy'])
                            && !empty($v['ActivityStrategy'])
                            && $v['ActivityStrategy'] != $params['ActivityStrategy']
                        ){
                            unset($couponArray[$k]);
                        }
                    }*/

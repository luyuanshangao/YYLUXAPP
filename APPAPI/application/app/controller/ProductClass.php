...
class ProductClass extends AppBase
{
    ...
    public function index(){
        ...
        $list =$this->classService->getClass($params);
        if(!empty($list['data'])){
            $list=  id_arr_key($list['data'],'id');
            $data=$ProductClassService->getIntegrationClass(['lang'=>$lang],$list);
        }
        return $data;
        ...
        try{
            ...
            if(!is_array($data['data'])){
                ...
                foreach($data['data'] as &$value){
                    $value['title_en']=!empty($value['Common'][$lang])?$value['Common'][$lang]:$value['title_en'];
                    $value['classimg']=!empty($value['classimg'])?$value['classimg']:'';
                }
            }
            return $data;
        }catch (Exception $e){
            ...
            return apiJosn(['code'=>200000004, 'msg'=>$e->getMessage()]);
        }
    ...
    public function getClassTree(){
        ...
        try{
            $data = $this->classService->getClassTree($params,$params['lang']);
            return apiReturn(['code'=>200, 'data'=>$data]);
        }catch (Exception $e){
            //错误日志
            Monlog::write(LOGS_MALL_API,'error',__METHOD__,__FUNCTION__,$params,null,$e->getMessage());

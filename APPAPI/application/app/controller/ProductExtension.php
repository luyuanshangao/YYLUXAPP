...
class ProductExtension extends Base
{
    ...
    public function getProductViewHistory(){
        ...
        try{
            ...
            return apiJosn(['code'=>1000000022, 'msg'=>$e->getMessage()]);
        }
     ...
     * 购物车一个或者多个的推荐数据
     */
    public function getCartRelatedProduct(){
        $params = request()->post();
        //参数校验
        $validate = $this->validate($params,$this->productParams->getProductRules());
        if(true !== $validate){
        ...
        try{
            ...
            foreach($product_ids as $id){
                $data = $this->extensionService->getRelatedProducts(['lang'=> $params['lang'],'product_id' => $id,'currency'=>$currency,'country'=>$country]);
                if(!empty($data)){
            ...
            if(!empty($result)){
                shuffle($result);
                $result = CommonLib::getRandArray($result,30);
            }
            return apiReturn(['code'=>200, 'data'=>$result]);
        }catch (Exception $e){
            //错误日志
    ...
    public function getRecommend(){
        try{
            ...
            return apiJosn(['code'=>20000000002, 'msg'=>$e->getMessage]);
        }
    }
}

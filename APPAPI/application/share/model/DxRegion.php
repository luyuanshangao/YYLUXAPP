...
class DxRegion extends Model{
     ...
     *
     */
    public function getHeaderCountry($params = array()){
        $query = $this->db->name('region');
        if(isset($params['isFocus']) && $params['isFocus'] != 1){
            $query->where(['isFocusCountry' => ['<>',1]]);
        }
        $query->where(['ParentID' => 0]);
        $result = $query->field(array('Name'=>true,'Code'=>true))->select();
        return $result;
    }
    ...
    public function getFocusCountry(){
        $query = $this->db->name('region');
        $query->where(['ParentID' => 0,"isFocusCountry"=>1]);
        $query->order("sort","asc");
        $query->field(array('Name'=>true,'Code'=>true));
        return $query->select();
    }
    ...
    public function getState($params = []){
        $data='';
        if(!empty($params['CountryCode'])&&!empty($params['ProvinceCode'])){
            $where['Code']=$params['CountryCode'];
            $Country= $this->db->name('region')->find();
            if(!empty($Country['_id'])){
                $where1['_id']=$Country['_id'];
                $where1['Code']=$params['ProvinceCode'];
                $Province= $this->db->name('region')->find();
                if(!empty($Province['Name'])){
                    $data=$Province['Name'];
                }
            }
        }
        if(empty($data)&&!empty($params['ProvinceCode'])){
            $data=$params['ProvinceCode'];
        }
        return $data;
    }
}
...
class ProductClassService extends BaseService
{
    ...
    public function getIntegrationClass($params){
        ...
        if(config('cache_switch_on')){
            $data = $this->redis->get(INTEGRATION_CLASS.'_'.$params['lang']);
        }
        if(empty($data)){
...
//                }
            ...
            $this->redis->set(INTEGRATION_CLASS.'_'.$params['lang'],$data,CACHE_DAY);
        }
    ...
    public function getClass($params){
        ...
        if(empty($result)){
            ...
            if(!empty($result)){
                $this->redis->set(CHILD_CATEGORIES_LIST_BY_.$pid.'_'.$params['lang'] , $result , CACHE_HOUR);
            }
        }
    ...
    public function getProductBrand($params){
        ...
        $result = $data = array();
        if(config('cache_switch_on')){
            $data = $this->redis->get(CATEGORY_BRAND_BY_.$class_id);
        }
        if(empty($data)){
            ...
            if(!empty($data)){
                foreach($data as $brands){
                    if(isset($brands['product_brand']) && !empty($brands['product_brand'])){
                        foreach($brands['product_brand'] as $bk => $brand){
                            if($brand['id'] == 1){
                                unset($brands['product_brand'][$bk]);
                            }
                        }
            ...
            if(!empty($data)){
                $this->redis->set(CATEGORY_BRAND_BY_.$class_id,$data,CACHE_DAY);
            }
        }
        return $data;
    }
    ...
    public function getClassInfo($params,$lang){
        ...
        if(!empty($data)){
            ...
            if(isset($data['type']) && $data['type'] != 1){
                $erp_id = isset($data['pdc_ids'][0]) ? $data['pdc_ids'][0] : $params['class_id'];
                $data = $this->classModel->getClassDetail(['id'=>(int)$erp_id],$lang);
            }
            if($data['level'] != 1){
...
//                        pr($third);die;
     ...
     * @return array
     */
    public function getFirstCategoryBrand($params){
        $this->classModel = new ProductClassModel();
        ...
        foreach($classData as $key => $value){
            ...
            if(!isset($brand['product_brand']) && empty($brand['product_brand'])){
                //类别映射
                if(!empty($value['pdc_ids'])){
                    ...
                    if(!empty($erp_brand)){
                        foreach($erp_brand as $erpData){
                            if(!empty($erpData['product_brand']) && is_array($erpData['product_brand'])){
                                $map_brand = array_merge($map_brand,$erpData['product_brand']);
                            }
                            break;
                        }
            ...
            //语种切换
            if($lang != DEFAULT_LANG){
                $classData[$key]['title_en'] = isset($value['Common'][$lang]) ? $value['Common'][$lang] : $value['title_en'];
            }
        }
        return $classData;
    }
                                ...
                                if(!empty($class['pdc_ids'])){
                                    ...
                                    foreach($mapList as $map){
                                        ...
                                        if($params['lang'] != DEFAULT_LANG){
                                            $data[$map['id']]['title'] = isset($map['Common'][$params['lang']]) && !empty($map['Common'][$params['lang']]) ?
                                                $map['Common'][$params['lang']] : $map['title_en'];
                                        }
                                    ...
                                    if($params['lang'] != DEFAULT_LANG){
                                        $data[$class['id']]['title'] = isset($class['Common'][$params['lang']]) && !empty($class['Common'][$params['lang']]) ?
     ...
     * 修复单个类别对应多个映射关系
     */
    public function ClassHasMoreMap(){
        $this->classModel = new ProductClassModel();
        //查找所有类别PDC
        $classList = $this->classModel->gePDCPdcids();
        foreach($classList as $class){
            $ids = CommonLib::supportArray($class['pdc_ids']);
            $detailMap = $this->classModel->getClassDetail(['id'=>$ids]);
            if(!empty($detailMap['pdc_ids'])){
                $count = count($detailMap['pdc_ids']);
                if($count > count($class['pdc_ids'])){
                    pr($class['id']);
                    pr($detailMap['pdc_ids']);
                    //合并
                    $newPdcids = array_merge($class['pdc_ids'],$detailMap['pdc_ids']);
                    //去重
                    $newPdcids = array_unique($newPdcids);
                    //更新
                    $this->classModel->updateClass(['pdc_ids'=>$newPdcids],['id'=>(int)$class['id']]);
                    pr($newPdcids);
                }
            }
        }
    ...
    public function handleClassStatus(){
        $this->classModel = new ProductClassModel();
        //查找所有类别映射为空的类别
        $classList = $this->classModel->getErpClassCloseStatus();
        foreach($classList as $class){
            $this->classModel->updateClass(['status'=>1],['id'=>(int)$class['id']]);
        }
    ...
    public function getClassDetail($params){
        return (new ProductClassModel())->getClassDetail(['id'=>(int)$params['class_id']],$params['lang']);
    }
    ...
    public function getCatetoryProductCount($params){
        ...
        switch($params['key']){
            case 'Coupon':
                ...
                if(!empty($couponSku)){
                    $productsData  = (new ProductModel())->selectProduct(['sku_code' => ['in',$couponSku],'field'=>['_id']]);
                    if(!empty($productsData)){
                        $products = CommonLib::getColumn('_id',$productsData);
                    }
                }
                break;
            case 'HotProduct':
                //每天定时任务跑的数据
                $productsData = (new ProductTopSellerModel())->getNewDataTopData(['products','_id'=>false]);
                $products = !empty($productsData['products']) ? $productsData['products'] : array();
                break;
            case 'FlashDeal':
                //FlashDeal当前进行中的活动，按分类统计产品数量
                $productsData  = (new ProductModel())->selectProduct(['activityProduct' => 1,'field'=>['_id']]);
                if(!empty($productsData)){
                    $products = CommonLib::getColumn('_id',$productsData);
                }
                break;
            case 'newArrivals':
                //新品数据，按分类统计产品数量
                $productsData  = (new ProductModel())->selectProduct(['isNewProduct' => 1,'field'=>['_id']]);
                if(!empty($productsData)){
                    $products = CommonLib::getColumn('_id',$productsData);
                }
                break;
        }
        $class_list = $this->groupByProductFirstCategory($products,$params['lang']);
        return $class_list;
    }
}

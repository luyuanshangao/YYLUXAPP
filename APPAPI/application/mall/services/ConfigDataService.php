...
//        Cache::set(Self::CACHE_KEY .'getCategoryHotWord', $keyWord, Self::CACHE_TIME * 100);
    ...
    public function getBrandsLogo(){
        ...
        return apiReturn(['code'=>200, 'data'=>$brand]);
    }
    ...
    public function getProductDataByKey($params){
        ...
        if(empty($products)){
            ...
            if(!empty($spus)){
                ...
                if(!empty($products)){
                    $this->redis->set(PRODUCT_CONFIG_DATA_BY_.$key. '_' . $lang.'_'.$country,$products,CACHE_HOUR);
                }
            }
        }
    ...
    public function getSystemConfigs($key){
        return $data = (new SysConfigModel())->getSysCofig($key);
    }
}
